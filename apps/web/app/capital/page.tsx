...
export default function CapitalPage() {
  ...
  const [correctionAction, setCorrectionAction] = useState<"edit" | "remove">(
    "edit",
  );
      ...
      Array.from(
        contributions.reduce(
          (totals, item) =>
            totals.set(
              item.member,
              (totals.get(item.member) ?? 0) + item.amount,
            ),
  ...
  useEffect(() => {
    fetch("/api/capital", { headers: actorHeaders() })
      .then(async (response) => {
        if (!response.ok)
          throw new Error(
            "Capital fund is not connected. Apply migration 0006 to enable the live ledger.",
          );
        ...
        setContributions(
          payload.contributions.map((item) => ({
            ...
            note: item.note ?? "Contribution",
          })),
        );
        setGoal(payload.goal);
        if (payload.goalProposal)
          setGoalProposal({
  ...
  async function recordContribution(event: React.FormEvent) {
    ...
    const response = await fetch("/api/capital", {
      ...
      body: JSON.stringify({ action: "contribution", amount: value, note }),
    });
    const payload = await response.json();
    if (!response.ok) {
      setMessage(payload.error ?? "Could not record contribution");
      return;
    }
    ...
    setAmount("");
    setShowRecord(false);
    setMessage("Contribution recorded in the live ledger.");
  }

  async function proposeGoal(event: React.FormEvent) {
    event.preventDefault();
    const value = Number(
      (event.currentTarget as HTMLFormElement).querySelector("input")?.value,
    );
    if (!value || value < total) return;
    const response = await fetch("/api/capital", {
      method: "POST",
      headers: actorHeaders(),
      body: JSON.stringify({ action: "goal", newAmount: value }),
    });
    const payload = await response.json();
    if (!response.ok) {
  ...
  async function approveGoal() {
    ...
    const response = await fetch("/api/capital", {
      ...
      body: JSON.stringify({
        action: "approve-goal",
        proposalId: goalProposal.id,
      }),
    });
    const payload = await response.json();
    if (!response.ok) {
      setMessage(payload.error ?? "Could not co-sign proposal");
      return;
    }
    if (payload.approved) {
      setGoal(Number(goalProposal.newAmount));
      setGoalProposal(null);
      setMessage("Goal approved and applied.");
    } else {
      setGoalProposal({
  ...
  async function proposeCorrection(event: React.FormEvent) {
    ...
    const response = await fetch("/api/capital", {
      ...
      body: JSON.stringify({
        action: "correction",
        contributionId: correctionFor.id,
        correctionAction,
        amount:
          correctionAction === "edit" ? Number(correctionAmount) : undefined,
        proposedNote: correctionNote,
      }),
    });
    const payload = await response.json();
    if (!response.ok) {
      setMessage(payload.error ?? "Could not create correction proposal");
      return;
    }
    ...
    setMessage(
      "Correction proposal created. Another governing member must co-sign.",
    );
  }
  ...
  return (
    <div className="portal-page">
      <div className="portal-page-head">
        <div>
          <p className="eyebrow">Governance · Shared capital</p>
          <h1 className="page-title">Capital fund</h1>
          <p className="page-subtitle">
            Durable contributions, accountable approvals, and a shared path to
            the next build.
          </p>
        </div>
      ...
      <div className="stat-grid" style={{ marginBottom: 18 }}>
        <div className="card stat-card">
          ...
          <div className="stat-value">{formatZar(total)}</div>
          <div className="stat-note">
            {contributions.length} posted contributions
          </div>
        </div>
        <div className="card stat-card">
          <span className="card-kicker">Current goal</span>
          <div className="stat-value">{formatZar(goal)}</div>
          <div className="stat-note">
            {Math.round((total / goal) * 100)}% funded
          </div>
        </div>
        <div className="card stat-card">
      ...
      <div className="grid-2">
        <section className="card card-pad">
          <div className="split">
            <div>
              <span className="card-kicker">Contributions ledger</span>
            ...
            <button
              ...
              onClick={() =>
                setMessage("Account details copied to your clipboard.")
              }
            >
                    ...
                    {can(actor?.role ?? "Viewer", "proposal") && (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setCorrectionAmount(String(item.amount));
          ...
          {corrections.length > 0 && (
            <div style={{ marginTop: 18 }}>
              <span className="card-kicker">Pending corrections</span>
              {corrections.map((proposal) => (
                <div className="list-row" key={proposal.id}>
                  <span>
                    <strong>
                      {proposal.action === "remove" ? "Removal" : "Edit"} ·
                      contribution #{proposal.contributionId}
                    </strong>
                    <small>
        ...
        <div className="stack">
          <section className="card card-pad">
            <div className="split">
              <div>
                ...
                <h2 className="card-title" style={{ marginTop: 6 }}>
                  Soshanguve acquisition
                </h2>
              </div>
              <span className="tag tag-blue">2026</span>
            </div>
            <div style={{ marginTop: 24 }}>
              <div className="split" style={{ marginBottom: 7, fontSize: 12 }}>
                <strong>{formatZar(total)}</strong>
                <span className="muted">of {formatZar(goal)}</span>
              </div>
          ...
          <section className="card card-pad">
            <span className="card-kicker">Leaderboard</span>
            <h2 className="card-title" style={{ marginTop: 6 }}>
              This year’s contributors
            </h2>
            {leaderboard.length ? (
              leaderboard.map(([name, amount], index) => (
                <div className="list-row" key={name}>
                  <span>
                    <strong>
                      {index + 1}. {name}
                    </strong>
                    <small>{index === 0 ? "Lead contributor" : "Member"}</small>
                  </span>
                  <span className="tag tag-green">{formatZar(amount)}</span>
                </div>
              ))
            ) : (
              <div className="empty-state" style={{ marginTop: 14 }}>
                Leaderboard appears after the first posted contribution.
              </div>
            )}
          </section>
        </div>
      ...
      {corrections.length > 0 && (
        <section className="card card-pad" style={{ marginTop: 18 }}>
          <span className="card-kicker">Governance audit</span>
          <h2 className="card-title" style={{ marginTop: 6 }}>
            Correction co-sign status
          </h2>
          {corrections.map((proposal) => (
            <div className="list-row" key={`audit-${proposal.id}`}>
              <span>
                <strong>
                  {proposal.action === "remove" ? "Removal" : "Edit"} ·
                  contribution #{proposal.contributionId}
                ...
                <small>
                  {proposal.proposedBy} · {proposal.approvals.length} signatures
                </small>
              </span>
              <span className="split">
      ...
      {can(actor?.role ?? "Viewer", "record") && showRecord && (
        <div className="modal-scrim" role="dialog" aria-modal="true">
          <form className="modal-card" onSubmit={recordContribution}>
            <div className="split">
              <div>
                <span className="card-kicker">Capital fund</span>
                <h2 className="card-title" style={{ marginTop: 6 }}>
                  Record contribution
                </h2>
              </div>
      ...
      {can(actor?.role ?? "Viewer", "proposal") && correctionFor && (
        <div className="modal-scrim" role="dialog" aria-modal="true">
          <form className="modal-card" onSubmit={proposeCorrection}>
            <div className="split">
              <div>
                <span className="card-kicker">Maker-checker</span>
                <h2 className="card-title" style={{ marginTop: 6 }}>
                  Propose correction
                </h2>
              </div>
            ...
            <p className="muted" style={{ fontSize: 12 }}>
              Contribution by {correctionFor.member} ·{" "}
              {formatZar(correctionFor.amount)}
            </p>
            <label className="field-label">
              ...
              <select
                ...
                onChange={(event) =>
                  setCorrectionAction(event.target.value as "edit" | "remove")
                }
              >
                <option value="edit">Edit contribution</option>
                <option value="remove">Remove contribution</option>
              </select>
            </label>
