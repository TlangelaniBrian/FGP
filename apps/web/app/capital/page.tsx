...
type Member = { id: number; name: string; role: Role; status: string };
...
export default function CapitalPage() {
  ...
  const activeMembers = members.filter((member) => member.status !== "suspended" && member.status !== "removed" && member.role !== "Viewer");
  ...
  useEffect(() => {
    ...
    fetch("/api/team").then((response) => response.ok ? response.json() : []).then((rows: Member[]) => setMembers(rows));
  }, []);
