...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [row] = await db.update(complianceDocuments).set({ status: body.status, pdfUrl: body.pdfUrl, submittedAt: body.status === "submitted" ? new Date() : undefined }).where(eq(complianceDocuments.id, id)).returning();
