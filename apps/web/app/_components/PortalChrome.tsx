...
export function PortalChrome({ mode, onModeChange }: { mode: VisualMode; onModeChange: (mode: VisualMode) => void }) {
  ...
  return (
    <>
        ...
        <div className="portal-toolbar">
          ...
          <div className="popover-anchor">
            <button className="user-trigger" onClick={() => setOpen(open === "user" ? null : "user")}>
              <span className="avatar">{actor?.initials ?? "?"}</span><span className="role-pill">{actor?.role ?? "No access"}</span><span className="chevron">⌄</span>
            </button>
      ...
      {actor?.role === "Viewer" && <div className="viewer-banner">⌑ Viewing as {actor.name} · Viewer — read-only.</div>}
    </>
  );
}
