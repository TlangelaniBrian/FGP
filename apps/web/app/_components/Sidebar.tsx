...
const nav = [
  ...
  { href: "/settings/tariffs", label: "Tariffs", icon: "receipt_long", capability: "EditTariffs" }, { href: "/settings", label: "Settings", icon: "settings" },
];
...
export function Sidebar({ projects }: { projects: Project[] }) {
  ...
  const visible = (item: (typeof nav)[number]) =>
    !("capability" in item) ||
  ...
  return <aside className="portal-sidebar">
    ...
    <nav className="portal-nav" aria-label="Main navigation">
      ...
      {nav.slice(6).filter(visible).map((item) => <Link key={item.href} href={item.href} className={isActive(item.href) ? "nav-item is-active" : "nav-item"}><PortalIcon name={item.icon} className="nav-icon" /><span>{item.label}</span></Link>)}
      {capabilities.includes("ManageTeam") && <Link href="/settings/scraper" className={isActive("/settings/scraper") ? "nav-item is-active" : "nav-item"}><PortalIcon name="sync" className="nav-icon" /><span>Scraper jobs</span></Link>}
    </nav>
