...
for _ in $(seq 1 240); do
  body="$(curl -fsS -m 5 http://localhost:3000 2>/dev/null | head -c 2000 || true)"
  if [[ "$body" == *"Pipeline overview"* ]]; then
    web_ready=true
    break
  fi
  sleep 2
...
if [[ "$web_ready" != true ]]; then
  echo "The FGP web service did not become reachable on http://localhost:3000." >&2
